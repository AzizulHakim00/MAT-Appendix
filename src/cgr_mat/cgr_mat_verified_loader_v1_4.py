...
print("Loading checksum-pinned CGR-MAT v1.4 source parts...")
...
patches = [
    ...
    (
        ...
        '    cohort["target"] = severity.loc[cohort.index].map({"uncomplicated": 0, "complicated": 1}).astype(int)\n\n    confirmed_mask = diagnosis.eq("appendicitis")\n    labelled_severity_mask = severity.isin({"complicated", "uncomplicated"})\n    confirmed_labeled = df.loc[confirmed_mask & labelled_severity_mask].copy()\n    confirmed_labeled["target_audit"] = (\n        severity.loc[confirmed_labeled.index]\n        .map({"uncomplicated": 0, "complicated": 1})\n        .astype(int)\n    )\n    confirmed_labeled["us_number_numeric_audit"] = pd.to_numeric(\n        confirmed_labeled["US_Number"], errors="coerce"\n    )\n    without_us_number = confirmed_labeled.loc[\n        confirmed_labeled["us_number_numeric_audit"].isna()\n    ].copy()\n    linked_multimodal = confirmed_labeled.loc[\n        confirmed_labeled["us_number_numeric_audit"].notna()\n    ].copy()\n\n    export_columns = [\n        c for c in ["Diagnosis", "Severity", "Management", "US_Number", "target_audit"]\n        if c in without_us_number.columns\n    ]\n    without_us_number[export_columns].to_csv(\n        RUN_DIR / "confirmed_appendicitis_without_us_number.csv", index=False\n    )\n\n    linkage_audit = pd.DataFrame([\n        {\n            "group": "all_confirmed_appendicitis_with_observed_severity",\n            "patients": int(len(confirmed_labeled)),\n            "complicated": int(confirmed_labeled["target_audit"].sum()),\n            "uncomplicated": int((confirmed_labeled["target_audit"] == 0).sum()),\n        },\n        {\n            "group": "multimodal_linked_confirmed_appendicitis",\n            "patients": int(len(linked_multimodal)),\n            "complicated": int(linked_multimodal["target_audit"].sum()),\n            "uncomplicated": int((linked_multimodal["target_audit"] == 0).sum()),\n        },\n        {\n            "group": "confirmed_appendicitis_without_us_number",\n            "patients": int(len(without_us_number)),\n            "complicated": int(without_us_number["target_audit"].sum()),\n            "uncomplicated": int((without_us_number["target_audit"] == 0).sum()),\n        },\n    ])\n    linkage_audit.to_csv(RUN_DIR / "prognostic_cohort_linkage_audit.csv", index=False)\n    print("\\nPROGNOSTIC COHORT LINKAGE AUDIT")\n    display(linkage_audit)\n    print(\n        "Note: 18 confirmed appendicitis cases have observed severity labels but no usable "\n        "US_Number. They are retained in the tabular audit and excluded only from the "\n        "raw-ultrasound multimodal cohort because image linkage is impossible."\n    )\n\n    duplicated_ids = cohort.loc[cohort["patient_id"].duplicated(keep=False), "patient_id"].unique().tolist()\n',
        "correct image-linkage cohort audit",
    ),
    (
        ...
        '    expected_total, expected_positive = 445, 116\n    actual_signature = (len(cohort), int(cohort["target"].sum()))\n    if actual_signature != (expected_total, expected_positive):\n        raise RuntimeError(\n            "Multimodal-linked prognostic cohort signature changed. "\n            f"Expected N={expected_total}, complicated={expected_positive}; "\n            f"found N={actual_signature[0]}, complicated={actual_signature[1]}."\n        )\n\n    full_signature = (len(confirmed_labeled), int(confirmed_labeled["target_audit"].sum()))\n    unlinked_signature = (len(without_us_number), int(without_us_number["target_audit"].sum()))\n    if full_signature != (463, 118) or unlinked_signature != (18, 2):\n        raise RuntimeError(\n            "Confirmed-appendicitis image-linkage audit changed. "\n            f"Expected full=(463, 118) and without_US_Number=(18, 2); "\n            f"found full={full_signature} and without_US_Number={unlinked_signature}."\n        )\n',
        "correct multimodal and tabular cohort signatures",
    ),
    (
        '        "confirmed_severity_cohort": int(len(cohort)),\n',
        '        "multimodal_linked_confirmed_appendicitis": int(len(cohort)),\n        "all_confirmed_appendicitis_with_observed_severity": int(len(confirmed_labeled)),\n        "confirmed_appendicitis_without_us_number": int(len(without_us_number)),\n',
...
print("✓ CGR-MAT v1.4 patched source assembled.")
print("Patched source SHA256:", patched_sha)
print("Launching the pipeline...")
exec(compile(source, f"cgr_mat_pipeline_v1_4@{SOURCE_COMMIT}", "exec"), globals(), globals())
